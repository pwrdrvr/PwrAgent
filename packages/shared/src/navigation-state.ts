...
import type { AutomationThreadSummary } from "./contracts/automations.js";
import type {
  AppServerBackendScope,
  AppServerThreadSummary,
  LinkedDirectorySummary,
} from "./contracts/normalized-app-server.js";
import {
  ...
  return JSON.stringify({
    ...
    threads: params.threads.map((thread) => ({
      ...
      subAgents: (thread.subAgents ?? []).map((subAgent) => ({
        ...
        preferredFastMode: subAgent.preferredFastMode ?? null,
      ...
      codexNativeSubAgents: (thread.codexNativeSubAgents ?? []).map((subAgent) => ({
        threadId: subAgent.threadId,
        title: subAgent.title,
        createdAt: subAgent.createdAt ?? null,
        updatedAt: subAgent.updatedAt ?? null,
        threadStatus: subAgent.threadStatus ?? null,
