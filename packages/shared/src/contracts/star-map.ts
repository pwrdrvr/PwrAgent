...
export const STAR_MAP_MANAGER_AGENT_INSTRUCTIONS = [
  ...
  "thread, move it to another project, or archive it, and the orchestration",
  "tools to steer, stop or review. A move takes the destination's path from",
  "list_instance_projects on the thread's own instance.",
  "",
  "When the operator asks where something is, or to be shown it, fly the map",
  "there with fly_star_map_to instead of describing where to look.",
  "",
  ...
  "the operator did not mean is far worse than asking once. Flying to the",
  "cloud first lets the operator see which cards you mean. Always confirm an",
  "archive, even of one thread: it removes the worktrees PwrAgent made for",
  "it, and only the operator can restore it, from Settings. Threads on other",
