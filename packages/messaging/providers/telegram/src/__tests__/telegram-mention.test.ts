...
describe("stripTelegramBotMention", () => {
  ...
  it("preserves args after the verb", () => {
    expect(
      stripTelegramBotMention(`@${USERNAME} resume thread-42`, USERNAME),
    ).toBe("resume thread-42");
  });

  it("matches case-insensitively (Telegram usernames are case-insensitive)", () => {
    expect(
      stripTelegramBotMention(`@${USERNAME.toLowerCase()} help`, USERNAME),
    ).toBe("help");
    expect(
      stripTelegramBotMention(`@${USERNAME.toUpperCase()} help`, USERNAME),
    ).toBe("help");
  });

  it("tolerates leading whitespace before the mention", () => {
    expect(stripTelegramBotMention(`   @${USERNAME} help`, USERNAME)).toBe(
      "help",
    );
  });
  ...
  it("returns undefined when the message doesn't start with the mention", () => {
    expect(
      stripTelegramBotMention(`hi @${USERNAME} help`, USERNAME),
    ).toBeUndefined();
  ...
  it("returns undefined when a longer username matches as a prefix", () => {
    ...
    expect(
      stripTelegramBotMention(`@${USERNAME}2 help`, USERNAME),
    ).toBeUndefined();
  });

  it("returns undefined when botUsername is unset", () => {
    expect(
      stripTelegramBotMention(`@${USERNAME} help`, undefined),
    ).toBeUndefined();
  });
});
