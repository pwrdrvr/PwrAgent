  ...
  return JSON.stringify({
    ...
    threads: params.threads.map((thread) => ({
      ...
      subAgents: (thread.subAgents ?? []).map((subAgent) => ({
        ...
        preferredReasoningEffort: subAgent.preferredReasoningEffort ?? null,
        monitorThreadId: subAgent.monitorThreadId ?? null,
        monitorTurnId: subAgent.monitorTurnId ?? null,
        lastMessage: subAgent.lastMessage ?? null,
        outcome: subAgent.outcome ?? null,
        completedAt: subAgent.completedAt ?? null,
        completionSource: subAgent.completionSource ?? null,
        monitorUsage: subAgent.monitorUsage ?? null,
        pollIntervalSeconds: subAgent.pollIntervalSeconds ?? null,
        heartbeatIntervalSeconds: subAgent.heartbeatIntervalSeconds ?? null,
        startupTimeoutSeconds: subAgent.startupTimeoutSeconds ?? null,
      })),
      ...
      turnFailureLog: (thread.turnFailureLog ?? []).map((entry) => ({
        id: entry.id,
        turnId: entry.turnId,
        error: entry.error,
        occurredAt: entry.occurredAt,
