...
**Progress 2026-06-07:** Added the `/agent` command, an Agent-only browse mode, stale-selection rejection for ordinary threads, and `targetKind: "agent_thread"` persistence through JSON and sqlite messaging stores. `/resume` keeps the existing all-thread behavior. Discord and Mattermost native command registration now include `agent`, and status cards/confirmation text label Agent bindings. `/agent --new` now reuses the existing `/resume --new` project/options/first-prompt flow, starts a default `Messaging Agent` via `startThread({ agent })` or materialized launchpads, and binds the surface as an Agent target. Zero-Agent create affordances and named Agent template selection remain follow-up work.
