...
Status: As built. The design in Part 3 shipped on both sides.

This is a historical decision record, landing after the fact. It was written
alongside PR #1733 and originally opened against that branch as PR #1756; the
branch was rebased repeatedly and then squash-merged to `main` as `86e04596c`,
which stranded the record twice over. Only ever one file, so it is carried here
directly onto `main` instead.

This record exists because the originally sketched design — rewrite the command
in `PreToolUse` to a wrapper that calls the desktop bridge — does not survive
contact with the shipped Codex CLI. The measurements below are the decision
input that redirected it.

The value now is the rationale, not the plan: what was measured, and why three
other approaches were rejected. None of that is recoverable from the shipped
code. See "As built" at the end for what landed.
...
- **Streaming.** This is a terminal reduction at the script-to-model boundary.
  ...
  through the seam, which partially covers the case. Not claimed as solved, and
  still not solved as built.
- **Upstreaming.** The fork shipped; an upstream PR was not attempted. The
  argument for one is unchanged: the seam replaces an existing dumb truncation
  rather than adding a new interception point. If it ever lands upstream, the
  fork thins to near nothing.

Resolved since: bundling went ahead, and the fork carries its own distribution
and Windows signing tooling.
...
should clamp it host-side. — Shipped as `max_output_tokens_ceiling`.
...
Every element of the Part 3 decision shipped, including both carried concerns.

## Codex side — `pwrdrvr/codex`, `agent/code-mode-output-reducer`

- `codex-rs/core/src/tools/code_mode/reducer.rs` — the seam, plus
  `reducer_tests.rs` and a `code_mode_output_reducer` integration suite.
- `codex-rs/features/src/feature_configs.rs` — `CodeModeOutputReducerToml`
  (descriptor path, re-read per reduction so the host can restart and rotate
  its token without restarting Codex) and `max_output_tokens_ceiling`.
- `docs/pwragent-distribution.md` and `scripts/pwragent-release/` — distribution
  and Windows signing, so bundling is real rather than planned.

The config comments carry the two load-bearing facts forward at the site that
depends on them: that the reducer is contacted from the Codex process, which is
not sandboxed, and that every failure mode falls back to built-in truncation so
a missing or wedged reducer never fails a turn.

## PwrAgent side — `agent/token-miser`

- `token-miser-hook-bridge.ts` — code-mode reducer descriptor, acceptance
  payloads, `TokenMiserCodeModeReductionOutput`, acceptance timeout.
- `token-miser-service.ts` — the `gpt-5.6-luna` ephemeral rewriter, with a
  pass-through policy that declines to reduce deliberate exact reads such as
  instruction files.

## Both carried concerns were handled

- **Untrusted framing.** The reducer wraps replacements in
  `UNTRUSTED_REPLACEMENT_HEADER` / `FOOTER`, fencing them in
  `<untrusted_reduced_output>` and telling the parent to treat the contents as
  data derived from tool output, never as instructions addressed to it. This
  was the one Part 1-era security concern the new design did not retire on its
  own, and it is closed at the seam.
- **Host budget ceiling.** `max_output_tokens_ceiling`, with the reasoning
  preserved in the config docs.

## What stays worth not re-deriving

Part 1 is the reason to keep this file. Two measurements there cost real time
and are invisible in the shipped code:

- The sandbox transport table — loopback is `EPERM` inside Codex's sandbox, and
  `AF_UNIX` works only when path-allowlisted.
- The `permissionDecision` anomaly — a `PreToolUse` rewrite applies only
  alongside `"allow"`, which no layer of the Codex source explains at either
  current `main` or the matching release tag. If Codex hook command rewriting is
  ever considered again for another feature, re-measure it on the target build
