...
The channel-neutral command surface — `resume`, `new`, `status`, `detach`, `help` — is defined in a single catalog at [`apps/desktop/src/main/messaging/core/messaging-command-catalog.ts`](../apps/desktop/src/main/messaging/core/messaging-command-catalog.ts).
