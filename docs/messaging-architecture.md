...
2. **The user-facing `/help` body.** `formatMessagingCommandHelpBody` derives the command list from the catalog so adding or renaming a verb in one place updates the help text everywhere it's rendered. The controller requests a Markdown body for providers whose capability profile advertises a Markdown dialect, and falls back to plain text for plain-only providers. Telegram converts that Markdown to Telegram HTML; Feishu/Lark renders it through the existing `lark_md` card path.
