...
There is no stable Windows `SHA256SUMS` URL on this branch. The publish step
uploads only the Windows `.exe` and `.blockmap`; the release's `SHA256SUMS`
asset is the Linux one.
