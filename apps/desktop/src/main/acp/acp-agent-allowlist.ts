...
/**
 * The `local-*` rules are **decorative for the local-discovery path**
 ...
 * `npx | uvx | binary`, so even if a registry payload for one of these
 * agents arrived tomorrow with `distributionKinds: ["local"]`, it would
 * be denied at the source check.
 *
 ...
 *   2. Forward-compat for future registry distributions of these CLIs —
 *      at that point each rule expands with `distributionKinds:
