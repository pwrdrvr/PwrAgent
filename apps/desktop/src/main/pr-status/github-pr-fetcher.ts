...
/**
 * How long a primed branch lookup stays usable. Short: it only has to survive
 * the gap between the batched fetch and the refresh it was fetched for, and a
 * stale primed value would be worse than a fresh subprocess.
 */
const DEFAULT_PRIMED_BRANCH_LOOKUP_TTL_MS = 30_000;

function primedBranchLookupKey(cwd: string, branch: string): string {
  return `${cwd}\0${branch}`;
}
...
export class GithubPrFetcher {
  ...
  /** Batched in-process branch lookups waiting to be consumed. See `primeBranchLookup`. */
  private readonly primedBranchLookups = new Map<
    string,
    { prs: PrSummary[]; expiresAt: number }
  >();
  ...
  async fetchAllPullRequestsForBranch(params: {
    ...
    branch: string;
    /**
     * Discovery may have primed this branch with a short-lived GraphQL result.
     * User/post-turn refreshes set this false because they promise an
     * authoritative read. The primed value is discarded so it cannot race the
    ...
    // sweep avoid one `gh` subprocess per branch without changing any of the
    // PR→thread attachment logic below this call.
    const primed = this.takePrimedBranchLookup(params.cwd, params.branch);
  ...
  /**
   * Seed results for specific (cwd, branch) lookups so the next
   * `fetchAllPullRequestsForBranch` for each one answers from memory instead of
   * spawning `gh`.
   *
  ...
  primeBranchLookup(
    ...
    ttlMs = DEFAULT_PRIMED_BRANCH_LOOKUP_TTL_MS,
  ): void {
    const expiresAt = Date.now() + ttlMs;
    for (const entry of entries) {
      this.primedBranchLookups.set(primedBranchLookupKey(entry.cwd, entry.branch), {
        prs: entry.prs,
        expiresAt,
      });
    }
  ...
  private takePrimedBranchLookup(
    cwd: string,
    branch: string,
  ): PrSummary[] | undefined {
    const key = primedBranchLookupKey(cwd, branch);
    const entry = this.primedBranchLookups.get(key);
    if (!entry) {
      return undefined;
    }
    this.primedBranchLookups.delete(key);
