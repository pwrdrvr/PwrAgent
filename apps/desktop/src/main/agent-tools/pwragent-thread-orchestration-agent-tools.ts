...
function descriptionForOperation(
  ...
  switch (operation) {
    ...
    case "steer_thread":
      return "Advise another PwrAgent thread. PwrAgent steers a matching active turn at the next tool boundary. If the target is idle or the turn changes before admission, PwrAgent preserves the guidance by starting a follow-up turn or queuing it behind the current turn. The result disposition is steered, started, or queued, so this tool never reports a fallback as steered. Use send_message_to_thread when a distinct new turn is required and stop_thread for an urgent interruption. Pass instanceId when known. Set includeRemote=false for local resolution. Reuse requestId only to retry the same steer. This thread cannot steer itself.";
    case "stop_thread":
