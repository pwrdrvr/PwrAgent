...
function descriptionForOperation(
  ...
  switch (operation) {
    ...
    case "get_thread_status":
      return "Read compact status and metadata for a PwrAgent thread. Results include linked directories, repository groups, pull requests, PR automation, pending handoffs, and workspace moves. Omit backend and threadId for the current thread. Pass instanceId for a known remote thread. Follow the returned prAutomation.guidance. autoFixActive means the inspected thread owns monitoring, not that another agent is repairing it. If Auto-fix started this thread's current turn, continue only the reported repair. Ignore unrelated prior context unless the user explicitly added it to this turn. When autoFixActive is true in another turn, avoid duplicate polling and follow the returned guidance. When it is false, do not assume PwrAgent will dispatch a repair turn.";
