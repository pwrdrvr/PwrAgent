...
describe("settings ipc", () => {
  ...
  it("rejects ACP installs when the selected distribution is not allowlisted", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "pwragent-settings-ipc-"));
    tempRoots.push(tempRoot);
        ...
        agents: [
          {
            ...
            distribution: {
              ...
              binary: {
                "linux-x64": {
                  archive: "https://example.com/gemini-linux.tar.gz",
                  checksum: "sha256:abc123",
                  cmd: "./gemini",
                },
                "darwin-aarch64": {
                  archive: "https://example.com/gemini-macos.tar.gz",
