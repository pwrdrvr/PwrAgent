...
describe("Git command contract", () => {
  ...
  it("passes shell metacharacters and Unicode as literal arguments", async () => {
    ...
    expect((await run(["config", "--get", "fixture.literal"])).stdout).toBe(value);
  });
  ...
  it("isolates snapshot staging from the real index and subsequent commands", async () => {
    ...
    const tree = (await run(["write-tree"], alternate)).stdout;
    expect((await run(["show", `${tree}:tracked.txt`])).stdout).toBe("snapshot content");
    expect(await readFile(index)).toEqual(originalIndex);
    expect((await run(["diff", "--cached", "--name-only"])).stdout).toBe("");
    expect((await run(["diff", "--name-only"])).stdout).toBe("tracked.txt");
  });

  // Confirmed defect, rather than a mock assertion that blesses trimming. The
  // consolidation PR must make this an ordinary passing regression test.
  it.fails("preserves the leading porcelain status column", async () => {
    await writeFile(path.join(repo, "tracked.txt"), "changed\n");
    expect((await run(["status", "--porcelain=v1", "-z"])).stdout).toBe(" M tracked.txt\0");
  });

  it.fails("preserves leading and trailing blob whitespace", async () => {
  ...
  it("streams past excluded untracked files before applying the result cap", async () => {
    ...
    expect((await run(["rev-parse", "--abbrev-ref", "HEAD"])).stdout).toBe("main");
  });
  ...
  it("keeps the worktree and real index when a required clean filter rejects a snapshot", async () => {
    ...
    const indexPath = (await runGitCommand(worktree, ["rev-parse", "--git-path", "index"], { env })).stdout;
