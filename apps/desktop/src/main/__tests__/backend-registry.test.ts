  ...
  return {
    ...
    setThreadModelSettings: async (settings: {
      ...
      if (settings.model && settings.reasoningEffort) {
        reasoningEffortsByModel[settings.model] = settings.reasoningEffort;
      }
      const next = {
        ...current,
        ...settings,
    ...
    listDirectoryLaunchpads: async () => Array.from(launchpads.values()),
...
`,
  ...
  it("prefers Terra as the default when Codex discovers the 5.6 family", async () => {
    const codexClient = new MockBackendClient({
      initializeResult: {
        serverInfo: { name: "Codex App Server", version: "1.0.0" },
        methods: ["thread/start", "turn/start"],
    ...
    expect(response.backends[0]?.launchpadOptions?.reasoningEfforts).toEqual([
      "low",
      "medium",
      "high",
      "xhigh",
    ...
    expect(codexClient.lastStartThreadParams?.model).toBe("gpt-5.6-terra");
    expect(codexClient.lastStartThreadParams?.reasoningEffort).toBe("medium");
  ...
  it("keeps fallback backend summary and thread-start defaults aligned", async () => {
    ...
    expect(models.find((model) => model.id === "gpt-5.6-sol")?.current).toBe(false);
    expect(models.find((model) => model.id === "gpt-5.6-terra")?.current).toBe(false);
    expect(models.find((model) => model.id === "gpt-5.6-luna")?.current).toBe(false);
    expect(models.find((model) => model.id === "gpt-5.5")?.current).toBe(true);
    expect(codexClient.lastStartThreadParams?.model).toBe("gpt-5.5");
  ...
  it("restores sticky launchpad reasoning when switching back to a model", async () => {
    const registry = new DesktopBackendRegistry({
      codexClient: new MockBackendClient({
        initializeResult: { methods: ["thread/start"] },
        models: [
    ...
    const terraUltra = await registry.updateDirectoryLaunchpad({
      directoryKey: "directory:/repo-a",
      patch: { model: "gpt-5.6-terra", reasoningEffort: "ultra" },
      stickySettingsChanged: true,
    });
    const luna = await registry.updateDirectoryLaunchpad({
      directoryKey: "directory:/repo-a",
      patch: { model: "gpt-5.6-luna" },
      stickySettingsChanged: true,
    });
    const terraRestored = await registry.updateDirectoryLaunchpad({
      directoryKey: "directory:/repo-a",
      patch: { model: "gpt-5.6-terra" },
      stickySettingsChanged: true,
    });

    expect(terraUltra.launchpad.reasoningEffort).toBe("ultra");
    expect(luna.launchpad.reasoningEffort).toBe("medium");
    expect(terraRestored.launchpad.reasoningEffort).toBe("ultra");
    expect(terraRestored.defaults.reasoningEffort).toBe("ultra");
    expect(
      terraRestored.defaults.providerSettings?.codex?.reasoningEffortsByModel,
    ).toMatchObject({
      "gpt-5.6-luna": "medium",
      "gpt-5.6-terra": "ultra",
    });
  ...
  it("restores thread reasoning when switching back to a model", async () => {
    const overlayStore = createOverlayStoreMock();
    const registry = new DesktopBackendRegistry({
      codexClient: new MockBackendClient({
        initializeResult: { methods: ["thread/start"] },
    ...
    const solUltra = await registry.setThreadModelSettings({
      backend: "codex",
      threadId: "thread-1",
      model: "gpt-5.6-sol",
      reasoningEffort: "ultra",
    });
    const luna = await registry.setThreadModelSettings({
      backend: "codex",
      threadId: "thread-1",
      model: "gpt-5.6-luna",
    });
    const solRestored = await registry.setThreadModelSettings({
      backend: "codex",
      threadId: "thread-1",
      model: "gpt-5.6-sol",
    });
    ...
    expect(luna.reasoningEffort).toBe("medium");
    expect(solRestored.reasoningEffort).toBe("ultra");
    expect(overlay?.reasoningEffortsByModel).toMatchObject({
      "gpt-5.6-luna": "medium",
      "gpt-5.6-sol": "ultra",
    });
  ...
  it("persists the Codex title helper as a system sub-agent with usage", async () => {
    ...
    expect(upsertUsageLineSpy).toHaveBeenCalledWith({
      line: expect.objectContaining({
        ...
        model: "gpt-5.6-luna",
        pricingRateId: "openai:2026-07-30:gpt-5.6-luna:standard",
        inputTokens: 100,
        cachedInputTokens: 20,
        uncachedInputTokens: 80,
        outputTokens: 10,
...
`,
  ...
  it("starts project-local handoffs from an explicit cwd instead of a stale parent linked directory", async () => {
    ...
    try {
      ...
      await overlayStore.upsertDirectoryLaunchpad({
        directoryKey: `directory:${expectedDir(targetRepoPath)}`,
        directoryKind: "directory",
        directoryLabel: "target-project",
        directoryPath: expectedDir(targetRepoPath),
        backend: "codex",
        executionMode: "default",
        prompt: "",
        workMode: "local",
      ...
      const response = await codexClient.emitRequest({
        ...
        params: {
          ...
          tool: "handoff_task",
          arguments: {
            task: "Investigate issue XYZ from the requested project checkout.",
            title: "Explicit project local handoff",
            workspaceMode: "project_local",
            cwd: targetRepoPath,
          },
        },
      ...
      expect(payload.workspace).toMatchObject({
        mode: "project_local",
        cwd: realTargetRepoPath,
        linkedDirectory: {
          kind: "local",
          label: "target-project",
          path: realTargetRepoPath,
        },
        git: {
          kind: "git_local",
          worktreeCreationAvailable: true,
    ...
    try {
      ...
      const response = await codexClient.emitRequest({
        ...
        params: {
          ...
          tool: "handoff_task",
          arguments: {
            task: "Investigate the export failure in the other project.",
            title: "Other project export failure",
            workspaceMode: "new_worktree",
            cwd: repoPath,
            branchName: "develop",
          },
  ...
  it("requires confirmation before a default new-worktree handoff uses an explicit untrusted cwd", async () => {
    ...
    try {
      ...
      expect(inputRequest.params.questions).toEqual([
        expect.objectContaining({
          id: "trust_directory",
          question: expect.stringContaining(expectedDir(repoPath)),
        }),
      ]);
      await registry.submitServerRequest({
        backend: "codex",
    ...
    try {
        ...
        overlayStore: createOverlayStoreMock({
          overlays: {
            "codex:ordinary-thread": {
              ...
              threadId: "ordinary-thread",
              executionMode: "full-access",
              codexEnvironmentRuntime: parentRuntime,
              extraLinkedDirectories: [parentDirectory],
            },
          },
        }),
      ...
      const response = await codexClient.emitRequest({
        ...
        params: {
          ...
          tool: "handoff_task",
          arguments: {
            task: "Fork a child for the other project.",
            title: "Other project fork",
            seedMode: "fork",
            workspaceMode: "new_worktree",
            cwd: repoPath,
          },
    ...
    try {
      ...
      const responsePromise = codexClient.emitRequest({
        ...
        params: {
          ...
          tool: "handoff_task",
          arguments: {
            task: "Investigate the trusted project.",
            title: "Trusted project handoff",
            workspaceMode: "project_local",
            cwd: repoPath,
          },
        },
      ...
      await expect(
        ...
        directoryKind: "directory",
        directoryLabel: "trusted-project",
        directoryPath: realRepoPath,
      });
      unsubscribe();
      await registry.close();
    } finally {
