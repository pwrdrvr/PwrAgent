...
describe("application discovery", () => {
  ...
  it("builds Ghostty AppleScript with an initial working directory", () => {
    expect(
      buildGhosttyAppleScriptArgs('/repo/.worktrees/feature "quoted"'),
    ).toEqual([
      "-e",
  ...
  it.skipIf(process.platform === "win32")(
    "opens VS Code source links with --goto line metadata",
    async () => {
  ...
  it.skipIf(process.platform === "win32")(
    "discovers IntelliJ IDEA from the idea launcher on PATH",
    async () => {
      ...
      const snapshot = await discoverDesktopApplications({
        env: { PATH: binDir },
      });
  ...
  it.skipIf(process.platform === "win32")(
    "opens IntelliJ IDEA source links with JetBrains line metadata",
    async () => {
      blockHostIntelliJDiscoveryPaths();
      const binDir = path.join(tempDir, "bin");
      const ideaPath = path.join(binDir, "idea");
  ...
  it.skipIf(process.platform === "win32")(
    "resolves the bundled VS Code CLI from an app-only install",
    async () => {
      ...
      await expect(
        resolveBundledApplicationCliPath(appPath, ["code"]),
      ).resolves.toBe(bundledCodePath);
    },
  );
});
...
describe("icns icon extraction", () => {
  it("detects the icns container magic", () => {
    expect(isIcnsBuffer(fakeIcns([{ type: "ic07", body: fakePng(128) }]))).toBe(
      true,
    );
