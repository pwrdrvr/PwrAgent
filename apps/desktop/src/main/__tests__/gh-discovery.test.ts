...
describe("GitHub CLI discovery", () => {
  it.skipIf(process.platform === "win32")(
    "returns usable candidates and selects a Homebrew gh outside PATH",
    async () => {
  ...
  it.skipIf(process.platform === "win32")(
    "dedupes PATH and well-known candidates that resolve to the same gh",
    async () => {
      ...
      expect(
        snapshot.candidates.filter(
          (candidate) => candidate.command === "/opt/homebrew/bin/gh",
        ),
      ).toHaveLength(1);
      expect(snapshot.selectedCommand).toBe("/opt/homebrew/bin/gh");
  ...
  it.skipIf(process.platform === "win32")(
    "shows checked well-known paths when no executable gh is found",
    async () => {
      const missingError = new Error("missing") as NodeJS.ErrnoException;
      missingError.code = "ENOENT";
  ...
  it.skipIf(process.platform === "win32")(
    "selects PWRAGENT_GH_COMMAND above config and auto discovery",
    async () => {
      accessMock.mockResolvedValue(undefined);
      execFileMock.mockImplementation(
        (
      ...
      expect(
        snapshot.candidates.find((candidate) => candidate.source === "env"),
      ).toMatchObject({
        selected: true,
        version: "2.90.0",
      });
