...
describe("TranscriptReview", () => {
  it("renders review summary metadata and prioritized findings", () => {
      ...
      <TranscriptReview
        ...
        entry={{
          ...
          output: {
            ...
            overall_explanation:
              "The live review result is currently rendered as plain text.",
    ...
    expect(screen.getByText("P1")).toHaveClass(
      "transcript-review__priority--p1",
    );
    expect(
      screen.getByText("Hydrate review transcript items"),
    ).toBeInTheDocument();
  ...
  it("hides raw entered-review protocol text when it matches the display label", () => {
    ...
    expect(screen.getByText("Review changes against main")).toBeInTheDocument();
    expect(
      screen.queryByText("changes against 'main'"),
  ...
  it("renders plain Codex review comments as review findings", () => {
    ...
    expect(screen.getByText("P2")).toHaveClass(
  ...
  it("renders full review comments as separate finding cards", () => {
    ...
    expect(
      screen.getByText("Only clear steer after it has actually been sent"),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
  ...
  it("colors every supported review severity and preserves absolute outside paths", () => {
    ...
    expect(
      screen.getByRole("link", {
        name: "/outside/repository/VeryLongOutsideFileName.ts",
      }),
    ).toHaveAttribute(
      "href",
      "file:///outside/repository/VeryLongOutsideFileName.ts:1",
    );
    expect(
      screen.getByRole("link", { name: "packages/app/high.ts" }),
    ).toBeInTheDocument();
  });
});
