...
describe("TranscriptCommandOutput", () => {
  ...
  it("renders command metadata and captured output", () => {
    ...
    expect(
      screen.getByText("dive@0.5.0 | Proprietary | deps: none"),
    ).toBeInTheDocument();
  ...
  it("truncates long output and expands on demand", () => {
    ...
    expect(
      screen.getAllByText(
        (_, element) => element?.textContent?.includes("line 12") ?? false,
      ).length,
    ).toBeGreaterThan(0);
    ...
    expect(
      screen.getAllByText(
        (_, element) => element?.textContent?.includes("line 15") ?? false,
      ).length,
    ).toBeGreaterThan(0);
