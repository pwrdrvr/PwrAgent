...
describe("collectEditedFileGroups clean-tree retirement", () => {
  const cleanTree = {
    dirtyFiles: 0,
    dirtyAdditions: 0,
    dirtyDeletions: 0,
  ...
  function accumulatedEntries(): AppServerThreadEntry[] {
    return [
      activityEntry({
        id: "a1",
        turnId: "turn-1",
  ...
  it("retires accumulated groups when the idle worktree reports a clean tree", () => {
    const groups = collectEditedFileGroups({
      entries: accumulatedEntries(),
      gitWorkingState: cleanTree,
    });
    expect(groups).toHaveLength(0);
  });

  it("keeps groups when the worktree is still dirty", () => {
    const groups = collectEditedFileGroups({
      entries: accumulatedEntries(),
      gitWorkingState: dirtyTree,
    });
    expect(groups).toHaveLength(1);
  });

  it("keeps groups when only untracked files remain (tree not clean)", () => {
    const groups = collectEditedFileGroups({
      entries: accumulatedEntries(),
      gitWorkingState: {
        dirtyFiles: 0,
  ...
  it("does not retire while a turn is active even if the signal reads clean", () => {
    const groups = collectEditedFileGroups({
      entries: accumulatedEntries(),
      activeTurnId: "turn-2",
      gitWorkingState: cleanTree,
  ...
  it("does not retire while a live pending entry is streaming", () => {
    const groups = collectEditedFileGroups({
      entries: accumulatedEntries(),
      livePendingEntry: activityEntry({
        id: "live-diff-turn-2",
        turnId: "turn-2",
        details: [fileDiffDetail({ path: "src/b.ts", additions: 2 })],
      }),
      gitWorkingState: cleanTree,
    });
    // The live group plus the accumulated group both survive.
    expect(groups.length).toBeGreaterThanOrEqual(1);
    expect(groups.some((group) => group.live)).toBe(true);
  });

  it("leaves the in-agent commit lifecycle untouched (committed group + clean tree)", () => {
    // A committed group that is still the last turn stays visible per #784,
    // and the clean-tree rule must not pre-empt that.
    const groups = collectEditedFileGroups({
      entries: [
        activityEntry({
          id: "a1",
          turnId: "turn-1",
          details: [
            fileDiffDetail({ path: "src/a.ts", additions: 4 }),
            commitDetail(),
          ],
        }),
      ],
  ...
  it("keeps groups when no working-state signal is available", () => {
    const groups = collectEditedFileGroups({
      entries: accumulatedEntries(),
    });
    expect(groups).toHaveLength(1);
