...
describe("App", () => {
  ...
  it("anchors an ephemeral sub-agent warning to its parent without offering an unavailable explorer", async () => {
    const agentEventListeners = new Set<(event: AgentEvent) => void>();
    const openToolOutputIncidentExplorerWindow = vi.fn(async () => ({
      opened: true as const,
    }));
    Object.defineProperty(window, "pwragent", {
      ...
      value: {
        getNavigationSnapshot: async () => ({
          ...
          fetchedAt: Date.now(),
          unchanged: false,
          inboxThreadKeys: ["codex:parent-thread"],
          threads: [{
            id: "parent-thread",
            title: "Release workflow monitor",
            ...
            subAgents: [{
              ...
              task: "Watch the release workflow",
              updatedAt: 1,
            }],
            updatedAt: 1,
          }],
    ...
    expect(await screen.findByText("Sub-agent large tool output")).toBeInTheDocument();
    expect(screen.getByText(/This came from an ephemeral sub-agent/)).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Open thread Codex thread" }))
      .toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Examine 1 case" })).toBeNull();
    expect(openToolOutputIncidentExplorerWindow).not.toHaveBeenCalled();
  });
