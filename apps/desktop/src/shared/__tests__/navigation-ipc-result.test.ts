...
import { offlineNavigationReadFailure, unwrapNavigationRead } from "../navigation-ipc-result";
...
it("transports an offline peer without a stack and restores the renderer rejection", () => {
  ...
  const payload = offlineNavigationReadFailure(error);
...
it("does not suppress unexpected failures or change successful responses", () => {
  expect(offlineNavigationReadFailure(new Error("broken projection"))).toBeUndefined();
  expect(offlineNavigationReadFailure(Object.assign(new Error("invalid query"), { code: "navigation_invalid_request" }))).toBeUndefined();
